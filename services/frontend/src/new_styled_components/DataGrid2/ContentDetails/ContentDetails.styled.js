...
const StyledContentDetails = ({activeContent, setActiveContent, folders, setFolders, callOnFocus}) => {
    const { t } = useTranslation();
    ...
    useEffect(() => {   
        ModuleService.read(activeContent.module).then((response) => {
  ...
  return (<>
        <ImageListItem >
            <img style={{ maxHeight:"160px", borderRadius:"8px 8px 0 0" }}
                src={ContentService.getImageUrl(activeContent)}
                alt=" "
                loading="lazy"
            />
            {getProgressBarComponent(activeContent.progress)}
            <Box sx={{ p: "8px", position: 'absolute', top: 0, width:'100%' }}><ETypeChip className={classes.chip1} type={activeContent.contentType}></ETypeChip></Box>
            <Box sx={{ p: "8px", position: 'absolute', top: 0, textAlign: 'right', width:'100%', fontSize:'12px', color:palette.neutrals.white }}>
                ...
                {activeContent.durationTime && Math.floor(activeContent.durationTime / 60) + "min"}
                </Box>
            </Box>
        </ImageListItem>
                ...
                <Box sx={{ display: 'flex', alignItems: 'center' }}>
                    <Box sx={{ flexGrow: 1, pl: "8px", py: "8px" }}>
                        ...
                        <Typography className={classes.typography14} style={{ color: palette.neutrals.darkestGrey }}>
                            {t("Author")}: {activeContent.owner.name} {activeContent.owner.surname}
                        </Typography>

                        <Typography className={classes.typography12}>
                            {t("Created")}: {F_getLocalTime(activeContent.createdAt)}
                        </Typography>
                    </Box>
                    <Box sx={{ width: '40px', height: '40px', mr: '8px' }} >
                        <OptionsButton iconbutton={+true} btns={getActionsForContent(activeContent)} size="large" sx={{borderRadius: '50%', width: 40, height: 40, p: 0, mx: 0}} />
                    </Box>
                    <Box sx={{ width: '40px', height: '40px', mr: '8px' }} >
                        <EIconButton onClick={() => {navigate(`/content/display/${activeContent._id}`)}} size="large" color="primary" sx={{borderRadius: '50%', width: 40, height: 40, p: 0, mx: 0}}>
            ...
            <Grid item xs={12} className="px-3" style={{ borderTop: '1px solid '+ palette.neutrals.white }}>
                ...
                { showMore && (<>
                    ...
                    <Typography className={classes.typography14} sx={{textAlign:'center'}}>
                        {activeContent.tags?.slice(0, 3).map((hashtag, index) => <span style={{paddingRight:'6px'}} key={index}>#{hashtag.name.split(' ').map(s => s.charAt(0).toUpperCase() + s.slice(1)).join('')}</span>)}
                        {activeContent.tags?.length > 3 && (tagExpanded? 
                            <>
                                {activeContent.tags?.slice(3).map((hashtag, index) => (<span key={index}>#{hashtag.name.split(' ').map(s => s.charAt(0).toUpperCase() + s.slice(1)).join('')} </span>))}
                                <span className={classes.showMore} onClick={() => setTagExpanded(false)}>show less</span>
                            </>:
                            <span className={classes.showMore} onClick={() => setTagExpanded(true)}>+{activeContent.tags.length - 3} more</span>
                        )}
                    </Typography>
                    {/* tags */}
                    <Box className="d-flex justify-content-center align-items-center my-1">
                        <ContentChips content={activeContent} elements={true} className={classes.chip}></ContentChips>
                    </Box>
                    ...
                    {<Grid container spacing={2} key={'Assigned'}>
                        ...
                        <Grid item xs={8}><Typography className={classes.typography14}>
                            {activeContent.assignedToPrograms===0?"No class":activeContent.assignedToPrograms===1?'1 program in a class':activeContent.assignedToPrograms+" classes' programs"}
                        </Typography></Grid>
                    </Grid>}
                    ...
                    {<Grid container spacing={2} key={'Source'}>
                        ...
                        <Typography className={classes.typography14}>
                            {activeContent.books?.length>0 ? activeContent.books.map((book, index) => <span key={index}>"{book.name}" {t('by')} {book.authors?.map((author, index) => <span key={index}>{author.name} {author.surname}{index<book.authors.length-1 && ', '}</span>)||t('unknown')}. {book.year}</span>) : t('Not found')}
                        </Typography>
                        </Grid>
                    </Grid>}
                        ...
                        <Grid item xs={8}>
                            <Typography className={classes.typography14}>
                                ...
                                <Typography className={classes.typography16} component="span">
                                    {<CircleIcon sx={{ fontSize:10, color: getStatusColor(activeContent.libraryStatus), mx: '10px' }}></CircleIcon>}
                                    {t(getStatusText(activeContent.libraryStatus))}
                                </Typography>
                            </Typography>
                            {['ACCEPTED','REJECTED'].includes(activeContent.libraryStatus) && <Typography className={classes.typography14}>
                                {activeContent.approvedInLibraryAt && t('on') +' '+ F_getLocalTime(activeContent.approvedInLibraryAt)} {t('by')} {contentModule?.name??t("Not found")}
                            </Typography>}
                            {activeContent.libraryStatusReason && activeContent.libraryStatus==='REJECTED'&& <Typography className={classes.typography14}>
                                {t("Provided reason")}: {t(activeContent.libraryStatusReason)}
                            </Typography>}
                        </Grid>
                    </Grid>}
                </>)}
        ...
        <AssignToModal open={showAssignToModal} setOpen={setShowAssignToModal} content={activeContent} />
    </>)
}

export default StyledContentDetails