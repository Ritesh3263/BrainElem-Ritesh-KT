...
RoleTable(props) {
    ...
    const columns = [
        ...
        { 
            field: 'name', cellClassName:'tableRowContent', headerName: t('Name'), width: 100, flex: 1,
            renderCell: (params) => (
                <>
                    <Avatar className="mr-3" src={`/img/user_icons_by_roles/${params.row.picture}`} alt="user-icon-avatar" /> {params.row.name}
                </>
            )
        },

        {
            field: 'role', cellClassName:'tableRowContent', headerName: t('Role'), width: 100, flex: 1,
            renderCell: (params) => (
                <>
                     {params.row.name}
                </>
