...
export default function ModuleManagersTable(props) {
    const{
        moduleManagers=[],
    }=props;
    ...
    const moduleManagersList = moduleManagers.length>0 ? moduleManagers.map((item,index)=>
        ({  id: index+1,
    ...
    const columns = [
        ...
        { field: 'action',
            ...
            renderCell: (params) =>(
            <IconButton color="secondary" size="small" className={`${classes.darkViolet}`} onClick={()=>{navigate(`/modules/managers/form/${params.row.userId}`)}}>
                <BsPencil/>
            </IconButton>
            )
        }
    ...
    return (
        <div style={{width: 'auto', height: 'auto'}} >
                <EDataGrid
                    ...
                    setRows={setRows}
                    originalData={moduleManagersList}
                />
        </div>
    );
}
