...
// reload - function to trigger data reload
function Row(props) {
  ...
  const onChangeHandler = (e, id) => {
    ...
    if(!trainee) return;
    if(e.target.checked){
      console.log(`Adding user with ID ${id}. There are ${users.length+1} users on the list.`)
      if(!users.includes(id)) setUsers(pre=>[...pre ,id])
    }else{
      console.log(`Removing user with ID ${id}. There are ${users.length-1} users on the list. `)
      ...
      <React.Fragment>
        ...
        <TableRow className='rowAlt' sx={{ '& > *': { border: 'none' } }} >
          <TableCell style={{padding:props.css}}>
            <StyledEIconButton onClick={() => setOpen(!open)} size="medium" color="primary">
          ...
          <TableCell style={{padding:props.css}} align='center' className='tableRowContent' hidden={!props.statusColumn}>
            {
              row?.status != undefined ?
                <Button
                  ...
                  eSize="xsmall"
                  className={`sts_fill btnStatus
                  ${row?.status === STATUSES.COMPLETED ? 'btnStatusCompleted'
                      : row?.status === STATUSES.NOT_COMPLETED ? 'btnStatusNotCompleted'
                        : row?.status === STATUSES.REQUEST_SENT ? 'btnStatusSent'
                          : row?.status === STATUSES.undefined ? 'btnStatusNoTest'
                            : 'btnStatusNoTest'}`}><BsFillCircleFill className='statusFill' />
              ...
              <Box sx={{ margin: 0 }} className='innerBcTable'>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "10px 0 20px 0" }} >
                  ...
                  <Box sx={{ width: '75px', paddingLeft: '10px' }}>
                    ...
                    <StyledButton eVariant="primary" eSize="xsmall" className="btnSaveSmall" {...row} disabled={users.length === 0}
                      onClick={()=>{setCalendarOpen(true)}} 
                    >
                        {users.length ? t("Send Request for {{number}} users", {number: users.length}) :  t("Send Request") }
                    </StyledButton>
                  ...
                  <TableBody className='tableRoleList '>
                    {row.trainee.map((trainee) => (
                      ...
                      <TableRow key={trainee.date} className="rowAltIn2">
                        ...
                        {trainee?.brainCoreTest?.status === STATUSES.COMPLETED ? <TableCell sx={{ width: '20%', paddingLeft: '25px' }} align='center'><BsFillCircleFill className='sts_fill completed' style={{ color: new_theme.palette.secondary.SGreen }} /></TableCell> :
                          trainee?.brainCoreTest?.status === STATUSES.NOT_COMPLETED ? <TableCell sx={{ width: '20%', paddingLeft: '25px' }} align='center'><BsFillCircleFill className='sts_fill not_completed' style={{ color: new_theme.palette.secondary.SRed }} /></TableCell> :
                            trainee.brainCoreTest?.status === STATUSES.REQUEST_SENT ? <TableCell sx={{ width: '20%', paddingLeft: '25px' }} align='center'><BsFillCircleFill className='sts_fill request_sent' style={{ color: new_theme.palette.secondary.Turquoise }} /></TableCell> :
                              <TableCell sx={{ width: '20%', paddingLeft: '25px' }} align='center'><BsFillCircleFill className='sts_fill no_test_taken' style={{ color: new_theme.palette.newSupplementary.NSupText }} /></TableCell>
                        }
