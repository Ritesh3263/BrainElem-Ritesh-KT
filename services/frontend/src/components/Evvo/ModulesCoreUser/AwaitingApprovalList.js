        ...
        <Paper key={index} sx={{p:2, m:1}} elevation={3}>
            <Box item xs={12} key={index}>
                <ESwitchWithTooltip
                    ...
                    ></ESwitchWithTooltip>
                <div>
                    <Typography variant="body1" style={{fontSize: "12px", display:"inline-block", marginLeft:"10px"}}>{`${t("Verification date:")} ${new Date(cert.verificationDate)}`}</Typography>
                </div>
                <div>
                    <Typography variant="body1" style={{fontSize: "12px", display:"inline-block", marginLeft:"10px"}}>{`${t("Blockchain status:")} ${cert.blockchainStatus? cert.blockchainNetworkId: 'none'}`}</Typography>
                </div>
                <div>
                    <Typography variant="body1" style={{fontSize: "12px", display:"inline-block", marginLeft:"10px"}}>{`${t("Internship status:")} ${cert.internshipStatus? 'Completed': 'none'}`}</Typography>
                </div>
                <div>
                    <Typography variant="body1" style={{fontSize: "12px", display:"inline-block", marginLeft:"10px"}}>{`${t("Module manager approval:")} ${cert.moduleManagerAproval? 'Approved': 'Pending'}`}</Typography>
                </div>
                {cert.moduleManagerAprovalDate && <Typography variant="body1" style={{fontSize: "12px", display:"inline-block", marginLeft:"10px"}}>{`${t("Module manager approval date:")} ${cert.moduleManagerAprovalDate}`}</Typography>}
            </Box>
        </Paper>
    ));
    })
