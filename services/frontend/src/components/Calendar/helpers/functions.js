const helperFunctions = {
    ...
    isSunday: function (d, m, y) {
        var day = new Date(y, m, d);
        return day.getDay() === 0;
    },
    isSaturday: function (d, m, y) {
        var day = new Date(y, m, d);
        return day.getDay() === 6;
    },
    decomposeRGBA: function (color) {
        if (!color)
            return null;
    ...
    getRGBAColorWithAlpha: function (color, alpha) {
        var rgba = this.decomposeRGBA(color);
        return rgba != null
    ...
    getFirstWeekDayOfMonth: function (cM, cY) {
        let toReturn = (new Date(cY, cM, 1).getDay())-1;
        if(toReturn <0){
            toReturn = 6;
        }
        return toReturn;
