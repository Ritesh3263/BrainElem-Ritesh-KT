...
export default function InspectorGradebook(){
        ...
        try{
            ...
            if(response.status===200){
                ...
                if(response.data.length>0){
                    ...
                    response.data[0].periodId = selectedPeriod || response.data[0].academicYear.periods.find(p=>new Date(p.startDate)<=new Date() && new Date(p.endDate)>=new Date())?._id||response.data[0].academicYear.periods[0]._id;
    ...
    const func12 = async(data,opt2)=>{
        ...
        try{
            data[0].periodId = selectedPeriod || class1.academicYear.periods.find(p=>new Date(p.startDate)<=new Date() && new Date(p.endDate)>=new Date())?._id||class1.academicYear.periods[0]._id;
    ...
    useEffect(()=>{
        ...
        setTableColumns(
            [
                { field: 'id',  _id: "BF_1", maxWidth:40, minWidth:40, headerClassName: classes.verticalText, headerName: 'ID', hide: false, sortable: false, disableColumnMenu: true},
