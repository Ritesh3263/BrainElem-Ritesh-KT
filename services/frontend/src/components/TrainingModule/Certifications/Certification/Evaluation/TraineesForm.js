...
export default function TraineesForm({studentFormHelper, setStudentFormHelper, sessionId}){
    ...
    useEffect(()=>{
        studentFormHelper?.studentId && CertificateService.readTraineeInSession(studentFormHelper?.studentId, sessionId).then(res=>{
            let currentDetail = res.data.certificates.find(x=>x.certificationSession === sessionId).details.find(y=>y.examiner === user.id)
            ...
            <CardContent>
                <Grid container spacing={1}>
                    <Grid item xs={12} className='d-flex flex-column'>
                        {detail?.competenceBlocks.map((cb, index)=>(
                                    ...
                                    <Slider
                                        ...
                                        onChange={(e,newVal)=>{
                                            setDetail(p=>{
                                                ...
                                                val.competenceBlocks[index].competences[i].grade = newVal;
                                                return val;
                                            })
                        ...
                        {detail?.externalCompetences?.length>0 && 
                            <Card className={"p-3 d-flex flex-column mt-3"} >
                                ...
                                {detail.externalCompetences.map((ec,j)=>(
                                    ...
                                    <Slider
                                        ...
                                        onChange={(e,newVal)=>{
                                            setDetail(p=>{
                                                let val = Object.assign({},p);
                                                val.externalCompetences[j].grade = newVal;
                                                return val;
                                            })
