...
export default function AcceptingCourseTable(props){
    const{
        awaiting=[],
        statusFilter,
        setStatusFilter
        ...
        { field: 'action',
            ...
            renderCell: (params) =>(
                <IconButton size="small" color="secondary"
                     onClick={()=>{navigate(`/accepting-library-course/${params.row.itemId}`)}}><BsPencil/>
                </IconButton>
            )
        }
    ];
    ...
    function switchStatus(status, toArchive){
        switch (status) {
            case "AWAITING":{
                return (<span className="text-muted"><Tooltip title={t("Course waiting for accepted")}><HourglassFullIcon/></Tooltip></span>);
            }
            // case "UPLOADED":{
            //     return (<span className="text-warning"><Tooltip title="Course was updated and waiting for accepted"><CloudUploadIcon/></Tooltip></span>);
            // }
            case "ACCEPTED":{
                if(toArchive === true){
                    return (<SvgIcon viewBox="0 0 32 32" component={DeleteIcon} title={t("Course waits to be archived")} />)
                }
                else{
                    return (<span className="text-success"><Tooltip title={t("Course was accepted")}><CloudDoneIcon/></Tooltip></span>);
                }
            }
            case "REJECTED":{
                return (<span className="text-danger"><Tooltip title={t("Course was rejected")}><CloudOffIcon/></Tooltip></span>);
            }
            case "PRIVATE":{
                return (<span><Tooltip title={t("Course is private")}><Chip label={t("private")} size="small"/></Tooltip></span>);
            }
            default :{
